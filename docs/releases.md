...
The release workflow does not rerun CI. It checks the tag, project version, and matching nonempty changelog entry; builds `linux/amd64` and `linux/arm64` images on their native runners (`ubuntu-24.04` and `ubuntu-24.04-arm`), then merges their digests into a versioned GHCR multi-platform image; smoke-tests its digest on the same native runner architectures; and, after both pass, creates a GitHub Release with notes and image references. Reruns preserve existing release notes.
